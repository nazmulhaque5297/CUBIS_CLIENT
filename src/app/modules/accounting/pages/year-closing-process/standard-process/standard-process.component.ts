...
@Component({
  selector: 'app-standard-process',
  templateUrl: './standard-process.component.html',
  styleUrls: ['./standard-process.component.css'],
})
export class StandardProcessComponent implements OnInit, OnDestroy {
  ...
  onCalculation(): void {
    if (confirm('Are you sure?')) {
      ...
      var IsAllBranch = false;
      if (!formValue?.BranchId) {
        IsAllBranch = true;
      }
      console.log('==', formValue?.BranchId, IsAllBranch);
      this.pService
        .CalculateProcess(formValue.AccountTypeId, IsAllBranch)
        .pipe(takeUntil(this.destroy$))
        .subscribe(
          (data) => {
            this.spinner.hide();
  ...
  public setParameter1(): void {
    ...
    if (fValue.BranchCode == '') {
      ...
      this.reportModel.ReportName = 'rptCsSBInterestReportAll';
    } else {
      this.reportModel.Values.push(
